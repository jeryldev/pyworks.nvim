...
function M.notebook_to_markdown(filepath)
    ...
    local cmd = string.format(
        "%s -m jupytext --to markdown --output - '%s' 2>/dev/null",
        python_path,
        filepath
    )
...
function M.notebook_to_python(filepath)
    ...
    local cmd = string.format(
        "%s -m jupytext --to py:percent --output - '%s' 2>/dev/null",
        python_path,
        filepath
    )
...
function M.sync_notebook_to_python(notebook_path, python_path)
    ...
    local cmd = string.format(
        "%s -m jupytext --to py:percent --output '%s' '%s'",
        python_cmd,
        python_path,
        notebook_path
    )
...
function M.sync_python_to_notebook(python_path, notebook_path)
    ...
    local cmd = string.format(
        "%s -m jupytext --to notebook --output '%s' '%s'",
        python_cmd,
        notebook_path,
        python_path
    )
