...
function M.has_ijulia()
    ...
    local cmd = string.format(
        '%s -e "using Pkg; exit(in(\"IJulia\", keys(Pkg.installed())) ? 0 : 1)" 2>/dev/null',
        julia_path
    )
...
function M.install_ijulia()
    ...
    local cmd = string.format(
        '%s -e "using Pkg; Pkg.add(\"IJulia\"); using IJulia"',
        julia_path
    )
...
function M.get_installed_packages()
    ...
    local cmd = string.format(
        '%s -e "using Pkg; for (name, _) in Pkg.installed() println(name) end" 2>/dev/null',
        julia_path
    )
