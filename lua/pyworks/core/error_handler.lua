...
function M.protected_call(func, error_prefix, ...)
	...
	if not ok then
		...
		vim.notify(string.format("❌ %s: %s", error_prefix or "Error", error_msg), vim.log.levels.ERROR)
		return false, nil
	end
	return true, result
end
...
function M.validate_filepath(filepath, operation)
	...
	if not filepath or filepath == "" then
		vim.notify(string.format("❌ Cannot %s: No file path provided", operation), vim.log.levels.ERROR)
		return nil
	end
	...
	if vim.fn.filereadable(filepath) ~= 1 then
		vim.notify(string.format("❌ Cannot %s: File not found: %s", operation, filepath), vim.log.levels.ERROR)
		return nil
	end
...
function M.validate_directory(dirpath, operation)
	...
	if not dirpath or dirpath == "" then
		vim.notify(string.format("❌ Cannot %s: No directory path provided", operation), vim.log.levels.ERROR)
		return nil
	end
	...
	if vim.fn.isdirectory(dirpath) ~= 1 then
		vim.notify(string.format("❌ Cannot %s: Directory not found: %s", operation, dirpath), vim.log.levels.ERROR)
		return nil
	end
...
function M.validate_packages(packages, language)
	...
	if not packages or type(packages) ~= "table" or #packages == 0 then
		vim.notify(string.format("❌ No %s packages specified", language), vim.log.levels.ERROR)
		return nil
	end
	...
	if #valid_packages == 0 then
		vim.notify(string.format("❌ No valid %s packages specified", language), vim.log.levels.ERROR)
		return nil
	end
