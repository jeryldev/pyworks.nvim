...
local function auto_init_molten(language)
    ...
    if vim.fn.exists(":MoltenInit") ~= 2 then
        local notifications = require("pyworks.core.notifications")
        notifications.notify(
            "Molten not available - install with :Lazy load molten-nvim",
            vim.log.levels.WARN
        )
        return
    end
    ...
    local notifications = require("pyworks.core.notifications")
    notifications.notify(
        string.format("Setting up %s kernel for execution...", language),
        vim.log.levels.INFO
    )
    ...
    if kernel then
        ...
        vim.defer_fn(function()
            ...
            if ok then
                ...
                local notifications = require("pyworks.core.notifications")
                notifications.notify(
                    string.format("Failed to auto-initialize kernel for %s. Press <leader>mi to initialize manually", language),
                    vim.log.levels.WARN
                )
            end
        ...
        if language == "julia" then
            notifications.notify(
                "Julia kernel not found. Ensure IJulia is installed.",
                vim.log.levels.WARN
            )
        elseif language == "r" then
            notifications.notify(
                "R kernel not found. Ensure IRkernel is installed.",
                vim.log.levels.WARN
            )
        else
            notifications.notify(
                string.format("No kernel found for %s", language),
                vim.log.levels.WARN
            )
        end
    end
