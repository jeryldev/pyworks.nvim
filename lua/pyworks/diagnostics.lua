...
function M.check_environment()
  ...
  for _, pkg in ipairs(packages) do
    ...
    local check_cmd = string.format("%s -c 'import %s; print(%s.__version__)' 2>/dev/null", check_python, import_name, import_name)
  ...
  if vim.fn.filereadable(rplugin_file) == 1 then
    ...
    table.insert(diagnostics, "Remote plugin registration: " .. (has_molten and "✓ molten registered" or "✗ molten not found"))
  else
...
function M.show_packages()
  local package_categories = {
    ["🔬 Data Science Essentials"] = {
      ...
      "statsmodels", "scikit-learn", "jupyter", "notebook"
    },
    ["🤖 Machine Learning"] = {
      "tensorflow", "keras", "torch", "torchvision", "xgboost", 
      "lightgbm", "catboost", "optuna", "mlflow"
    },
    ["📊 Visualization"] = {
      "plotly", "bokeh", "altair", "holoviews", "dash", 
      "streamlit", "gradio", "panel"
    },
    ["🗄️ Data Engineering"] = {
      "dask", "ray", "vaex", "polars", "pyarrow", 
      "sqlalchemy", "pymongo", "redis-py"
    },
    ["🌐 Web & APIs"] = {
      "requests", "httpx", "beautifulsoup4", "scrapy", 
      "fastapi", "flask", "django", "aiohttp"
    },
    ["🔧 Utilities"] = {
    ...
    ["📚 NLP & Text"] = {
      ...
      "openai", "anthropic", "gensim", "textblob"
    },
    ["🖼️ Computer Vision"] = {
      "opencv-python", "pillow", "scikit-image", "albumentations",
      "detectron2", "ultralytics", "mediapipe"
    }
  }
