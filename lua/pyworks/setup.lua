...
M.project_templates = {
  {
    ...
    optional = {
      ...
      "scipy", "statsmodels", "plotly", "notebook", "tensorflow",
      "torch", "torchvision"
    },
  ...
  },
  {
    ...
    essential = {"pynvim"},
    optional = {
      "fastapi", "uvicorn[standard]", "flask", "django",
      "sqlalchemy", "alembic", "pydantic", "python-dotenv",
      "requests", "httpx", "pytest", "black", "ruff"
    },
  ...
  },
  {
    ...
    optional = {
      ...
      "python-dotenv", "rich", "typer", "click"
    },
    create_nvim_lua = false,
  },
  {
    ...
    optional = {
      ...
      "schedule", "python-dotenv", "rich", "typer"
    },
    create_nvim_lua = false,
  },
  {
...
function M.setup_project()
  ...
  if vim.fn.isdirectory(venv_path) == 0 then
    ...
    local msg = has_uv 
      and "No .venv found. Create virtual environment with uv?" 
  ...
  if not current_python:match(venv_path) then
    ...
    vim.notify("Note: This works the same whether you created the venv with uv or python -m venv", vim.log.levels.INFO)
  end
  ...
  if #template.optional > 0 then
    ...
    if #missing_optional > 0 then
      ...
      local pkg_choice = vim.fn.confirm("Install project packages now?", "&Yes\n&No (install later)\n&Show list", 1)
...
if command -v uv &> /dev/null; then
  ...
  local script_content = string.format(install_script, 
    ...
    on_exit = function(_, exit_code)
      if exit_code == 0 then
        vim.schedule(function()
          vim.notify("✓ Packages installed successfully!" .. (has_uv and " (using uv)" or ""), vim.log.levels.INFO)
        end)
      else
        vim.schedule(function()
          vim.notify("⚠ Some packages failed to install. Run :PyworksCheck for details.", vim.log.levels.WARN)
        end)
      end
