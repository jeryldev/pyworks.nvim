...
describe("detector", function()
	...
	describe("kernel name safety", function()
		it("should only allow safe kernel names", function()
			local safe_pattern = "^[%w%-_%.]+$"
			assert.is_truthy(("python3"):match(safe_pattern))
			assert.is_truthy(("my_project_python"):match(safe_pattern))
			assert.is_truthy(("python-3.12"):match(safe_pattern))
			assert.is_falsy(("python3 | lua vim.cmd('!rm -rf /')"):match(safe_pattern))
